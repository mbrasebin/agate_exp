...
public class ProjectedShadowExec {
	...
	public static void main(String[] args) throws Exception {
		...
		for (IFeature feat : buildings) {
			IMultiSurface<IOrientableSurface> ims = Util.detectVertical(FromGeomToSurface.convertGeom(feat.getGeom()),
					0.2);
		...
		feat.setRepresentation(new TexturedSurface(feat,
				TextureManager.textureLoading("C:/Users/m.brasebin/EspaceTravail/Donnees/tmp/ombrage/background.png"),
	...
	public static List<IGeometry> generateGeometries(double azimuth, double zenith, IFeatureCollection<IFeature> proj,
			IFeatureCollection<IFeature> zone, IEnvelope env) {
		...
		List<IGeometry> lGeom2 = ProjectedShadow.process(proj, zone, (IPolygon) env.getGeom(), new Vecteur(-x, -y, -z),
				0, 15, ProjectedShadow.POSSIBLE_RESULT.PROJECTED_VOLUME, true);
